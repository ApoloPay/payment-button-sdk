...
class WC_Gateway_Apolo_Pay extends WC_Payment_Gateway {
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'webhook_url_info' => array(
                'title' => __( 'Webhook URL', 'apolo-pay' ),
                'type'  => 'webhook_url_info',
            ),
            'webhook_secret' => array(
                'title'       => __( 'Webhook Secret', 'apolo-pay' ),
                'type'        => 'password',
                'description' => __( 'Secret generated by ApoloPay. Go to Dashboard → Payment Button → Webhook → enter the URL above and copy the generated secret here.', 'apolo-pay' ),
                'default'     => '',
                'desc_tip'    => false,
            ),
        );
    ...
    public function generate_webhook_url_info_html( $key, $data ) {
        ...
        <tr valign="top">
            <th scope="row" class="titledesc">
                <label><?php esc_html_e( 'Webhook URL', 'apolo-pay' ); ?></label>
            </th>
            <td class="forminp">
                <fieldset>
                    <code style="
                    ...
                    <p class="description">
                        <?php esc_html_e( 'Copy this URL and paste it in your ApoloPay Dashboard → Payment Button Settings → Webhook URL. After saving, copy the generated Webhook Secret into the field below.', 'apolo-pay' ); ?>
                    </p>
                </fieldset>
            </td>
        </tr>
    ...
    public function process_payment( $order_id ) {
        ...
        $process_id = isset( $_POST['apolo_process_id'] ) ? sanitize_text_field( $_POST['apolo_process_id'] ) : '';
        $logger     = wc_get_logger();
        $context    = array( 'source' => 'apolo-pay' );

        if ( empty( $process_id ) ) {
            wc_add_notice(
                __( 'No se pudo verificar la transacción de Apolo Pay. Por favor, intenta de nuevo.', 'apolo-pay' ),
                'error'
            );
        ...
        if ( $pending_webhook ) {
            ...
            $order->add_order_note(
                sprintf(
                    __( 'Pago confirmado por ApoloPay (webhook recibido antes de crear la orden). Process ID: %s', 'apolo-pay' ),
                    $process_id
                )
            );
        ...
        $order->update_status(
            ...
            sprintf(
                ...
                __( 'Pago iniciado vía Apolo Pay. En espera de confirmación del webhook. Process ID: %s', 'apolo-pay' ),
                $process_id
            )
        );
    ...
    public function handle_webhook() {
        ...
        $signature = isset( $_SERVER['HTTP_X_SIGNATURE'] )
            ? sanitize_text_field( $_SERVER['HTTP_X_SIGNATURE'] )
            : '';

        $timestamp = isset( $_SERVER['HTTP_X_TIMESTAMP'] )
            ? sanitize_text_field( $_SERVER['HTTP_X_TIMESTAMP'] )
            : '';
        ...
        $logger->info(
            sprintf(
                "[WEBHOOK RECEIVED]\n  IP: %s\n  X-Signature: %s\n  X-Timestamp: %s\n  Payload: %s",
                isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( $_SERVER['REMOTE_ADDR'] ) : 'unknown',
                $signature ?: '(none)',
                $timestamp ?: '(none)',
                $payload ?: '(empty)'
            ),
        ...
        if ( ! $order ) {
            $orders = wc_get_orders( array(
                'meta_key'   => '_apolo_process_id',
                'meta_value' => $process_id,
                'limit'      => 1,
        ...
        $order->add_order_note(
            sprintf(
                /* translators: %s: ApoloPay process ID */
                __( 'Pago confirmado por webhook de ApoloPay. Process ID: %s', 'apolo-pay' ),
                $process_id
            )
        );
